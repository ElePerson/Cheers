...
- **一个文件可承载多个 bot。** 每个 `[accounts.<id>...]` 块是一个 bot；`<id>` 是本地
  标签（字母、数字、`_`、`-`），必须与你传给守护进程的 `--name` 一致。
...
| 键           | 类型   | 默认         | 含义 |
|--------------|--------|--------------|------|
| `name`       | string | 无           | 守护进程显示名（日志用）。 |
| `home_dir`   | string | 无           | 状态 + 日志的基目录。会被 `--home` / `CHEERS_ACP_HOME` 覆盖。 |
| `state_path` | string | `state.json` | 守护进程持久化运行态的位置。 |
| `log_dir`    | string | `logs`       | 滚动日志目录。 |
