...
from app.features.bot_runtime.orchestrator.secrets import extract_secret_refs, load_user_secrets
...
logger = logging.getLogger("app.features.bot_runtime.pipeline.bot.ingest")
...
class IngestStage(Stage[BotRunContext]):
    ...
    def _wrap_adapter_factory(ctx: BotRunContext, overrides: _PromptOverrides) -> None:
        ...
        async def wrapped(bot_id: str) -> BotAdapter:
            ...
            if override:
                from app.features.bot_runtime.orchestrator.adapter_resolver import (
