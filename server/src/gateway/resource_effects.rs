...
fn spawn_created_message_effects(state: &AppState, author_bot_id: Uuid, created: Value) {
    ...
    tokio::spawn(async move {
        ...
        // Web Push the @mentioned humans (kind=mention) — bots mention people
        // via post_message, and those people may be away from the tab. Already
        // off the critical path here; delivery is best-effort.
        if let Some(sender) = web_push {
            let human_mentions: Vec<String> = created
                ...
                .map(|arr| {
                    arr.iter()
                        .filter(|m| {
                            m.get("member_type").and_then(Value::as_str) == Some("user")
                        })
